...
void mCommandFunction32##NAME(Context* context, CommandInfo info); \
void mCommandFunction16##NAME(Context* context, CommandInfo info);

#define MCOMMAND_MRM_FUNCTION(NAME, D, W, F8, F16, F32)\
void mCommandFunction16##NAME(Context* context, CommandInfo info) {\
    uint8_t mrmByte = read8u(context);\
    uint8_t* destination = (uint8_t*)readAddressMRM16(context, mrmByte);\
    if (W && (info.command & 0x0001)) {\
        uint8_t* source = (uint8_t*)readRegisterMRM16(context, mrmByte);\
...
void mCommandFunction32##NAME(Context* context, CommandInfo info) {\
    uint8_t mrmByte = read8u(context);\
    uint8_t* source = (uint8_t*)readRegisterMRM32(context, mrmByte);\
    uint8_t* destination = (uint8_t*)readAddressMRM32(context, mrmByte);\
    if (W && (info.command & 0x0001)) {\
